...
def evaluate():
    parser = build_arg_parser("Evaluate V4 on test2 restricted 100-image gallery")
    ...
    emb_path = Path(cfg["embedding_dir"]) / "test2_v4_embeddings.npz"
    ...
    out_path = Path(cfg["results_dir"]) / "evaluation_v4.txt"
    with open(out_path, "w", encoding="utf-8") as handle:
        handle.write("=== V4 REMOTECLIP LORA EVALUATION ON TEST2 PAIRS ===\n")
    ...
    with open(Path(cfg["results_dir"]) / "metrics_test2_v4.json", "w", encoding="utf-8") as handle:
