...
def encode_gallery(model, sar_head, cfg, device):
    cache_path = Path(cfg["embedding_dir"]) / "combined_gallery_v5.npz"
    ...
    if len(ds) == 0:
        print("Warning: Gallery dataset is empty.")
        return np.zeros((0, 768), dtype="float32"), np.zeros((0, int(cfg["projection_dim"])), dtype="float32"), []
...
def evaluate():
    parser = build_arg_parser("Evaluate V5 test2 queries against all SAR images")
    ...
    if len(gallery_names) == 0:
        print("Cannot run full evaluation because gallery is empty.")
        return
    ...
    out_path = Path(cfg["results_dir"]) / "evaluation_v5_against_all.txt"
    with open(out_path, "w", encoding="utf-8") as handle:
        handle.write("=== V5 DINOV2 LORA EVALUATION ON COMBINED GALLERY ===\n")
    ...
    with open(Path(cfg["results_dir"]) / "metrics_combined_gallery_v5.json", "w", encoding="utf-8") as handle:
