...
def train():
    parser = build_arg_parser("Train V5 DINOv2 LoRA + Projection Head")
    ...
    try:
        ...
        print(f"Warning: Could not open dataset file: {e}")
        print("Please ensure dataset and metadata files are prepared.")
        return
    ...
    model = load_dinov2_v5(cfg, device, load_adapters=False)
    opt_head_dim = 768
    opt_head = ProjectionHead(input_dim=opt_head_dim, output_dim=int(cfg["projection_dim"])).to(device)
    sar_head = ProjectionHead(input_dim=opt_head_dim, output_dim=int(cfg["projection_dim"])).to(device)
    ...
    log_path = Path(cfg["results_dir"]) / "train_v5_log.json"
